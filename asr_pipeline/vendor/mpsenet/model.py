"""Vendored from MP-SENet/models/model.py.

Modifications relative to upstream:
- Imports rewritten to be local (`.transformer`, `.utils`).
- Removed `from pesq import pesq` and `from joblib import Parallel, delayed`.
- Removed training-only helpers (`phase_losses`, `anti_wrapping_function`,
  `pesq_score`, `eval_pesq`) since this package only performs inference.
"""
