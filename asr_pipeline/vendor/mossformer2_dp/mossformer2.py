...
class Mossformer2Wrapper(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        # VENDOR PATCH (2026-07-18, the ONE deviation from upstream): upstream
        # auto-moved the model to cuda here and printed it. The separation
        # stage owns device placement (phase-major, one model on GPU at a
        # time), so an __init__-time cuda grab fights the pipeline. Removed:
