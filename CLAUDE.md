...
4. **separation** — SepFormer 128k checkpoint by default (dataclass defaults and `configs/default.yaml` agree; a pin test enforces that *consistency*, not the literal `128k` — the checkpoint is swappable by editing both the dataclass default and `default.yaml`); runs on overlap fragments only.
...
7. **transcription** — `whisper` / `whisperx` backends; default = WhisperX `large-v2`. Alignment is per-language: `align_model_name=None` (default) lets WhisperX pick its per-language wav2vec2 default — for `pl` that is `jonatasgrosman/wav2vec2-large-xlsr-53-polish` (unchanged), set explicitly to override. English preset at `asr_pipeline/configs/english.yaml`. (rationale in `asr_pipeline/configs/README.md`).
