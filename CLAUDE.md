...
- `PolSESS_C_128_16kHz` — 16 kHz twin of 128_v2 (same six-run recipe, `dataSources_v3`, `outputFreq = 16000`; generated 2026-09-07/08). Same layout: `train/` 128,000 rows interleaved indoor/outdoor, `val/` = 1,000-item stratified subset hardlinked from `val_big/` (12,800), `test/` 12,800. `download_dataset.sh` does not know this corpus. That archive carries only `train/`/`val/`/`test/` - **no `val_big/`**.
...
- `polsess-separation-16kHz-128k` — 128k finals on the 16 kHz corpus (`PolSESS_C_128_16kHz`). Separate project on purpose: SI-SDR at 16 kHz is a different number on a different signal, not comparable with the 8 kHz rows.
...
- `tf_mossformer` (Zhao et al. 2026, arXiv:2607.21128): TF separator with convolution-gated local + global attention. **Re-implemented from the paper — no upstream code exists**; never call it "vendored". `tf_locoformer_blocks.py` is a deliberate separate copy of the Apache-2.0 TF-Locoformer file in `asr_pipeline/vendor/` (`models/` must not import from the liftable pipeline package); `local_global_attention.py` and `__init__.py` are original. Our sizes run ~2.5% above the paper's M/L rows because the paper's own rows are mutually inconsistent — quote the caveat wherever the size appears; reconciliation in `thesis/x_notes/tf_mossformer/tf_mossformer_spec.md` §4. Knobs follow paper Table 1 (`D`, `num_blocks`, `ffn_hidden_dim`, `conv_kernel_size`/`conv_stride`, `n_heads`, `num_groups`, `window_t`/`window_f`, `gate_kernel_size`, `n_fft`/`hop_length` in **samples**, `attn_dropout`; sweep key `dropout` → `attn_dropout`). Its configs (`experiments/tf_mossformer/{s,m,l}_8k.yaml` 16k rows; `{s,m}_128k_final.yaml` the 8 kHz 128k finals and `s_128k_16kHz_final.yaml` the 16 kHz one, all on the all-8 no-curriculum protocol with ES patience 8 instead of the paper's 10) are the only ones on the paper's recipe (`adamw`, `warmup_steps: 4000`, wd 1e-2). Plateau patience: the paper's is 3, but every run since commit c111f8d (2026-09-08) has `lr_patience: 1` — YAML headers that still say "patience 3" are stale (only `16k_tf_mossformer_s_42_originalLR` ran with 3). The paper's bs 4 / lr 1e-3 doesn't fit 12 GB, so they run the batch that fits with LR scaled linearly (S bs 2 / 5e-4, M and L bs 1 / 2.5e-4, S at 16 kHz bs 1 / 2.5e-4, no accumulation — author ruling 2026-09-07). 4070: S ≈ 4.4 GiB and ~29 min per 16k epoch, M 6.1 GiB, L 9.2 GiB. 3080 at 16 kHz: S bs 1 compiled 4.4 GiB, ~6.5 h per 128k epoch (bs 2 spills past 10 GB).

**Inference package (`pyproject.toml`, `models/inference.py`):** `models/` is also published as the pip-installable `polsess-models` distribution, import name `polsess_models` — hatchling maps the directory, nothing is copied, and code in this repository keeps importing `models` from the working directory. Other projects (DGSS-ASR, the extracted ASR pipeline) consume *inference bundles* (`weights.safetensors` + `separator.json`) written by `scripts/export_separator.py` and read by `load_separator`; `load_model_for_inference` (training checkpoints) lives in the same file and is re-exported by `utils`. Code under `models/` uses relative imports only and nothing from `config` / `utils` / `training` (`tests/test_polsess_models_package.py` enforces it); the Mamba family and `factory.py` are excluded from the wheel. A consumer-visible change in `models/` means bumping `models.__version__` and tagging `polsess-models-vX.Y.Z` — rules in `models/README.md`.
...
- **torch.compile:** auto on Linux (~10-20% speedup); loading handles the `_orig_mod` prefix; skipped for Mamba models. `mossformer2` and `tf_mossformer` compile with `dynamic=False` (their rotary block can't be lowered under symbolic shapes) — fixed-length crops compile once, a new length triggers a one-time recompile. Dispatch: `compile_for_model_type` in `utils/model_utils.py`. The eval-mode / no_grad / fp32 validation forward is a *second* compiled graph; `Trainer._preflight_validation` runs it on one batch per validation loader before epoch 1, so a compile failure there costs seconds instead of an epoch (torch 2.11's Inductor could not compile TF-MossFormer's eval graph at 16 kHz geometry; 2.14 can).
...
**Main (`venv/`)** — torch 2.14.0+cu130, triton 3.8, cuDNN 9.24, speechbrain 1.1.1, mamba-ssm 2.3.2.post1 and causal-conv1d 1.7.0 built from source. All models except SPMamba3. Alias: `polsess_venv`; `webapp/run.sh` uses it. On the 3080 machine this is the former `venv_t214/` (upgrade candidate built 2026-09-08: eval parity verified to 1e-5 dB, training throughput +5–18 % on fp16 models and TF-MossFormer, flat on MossFormer2/SPMamba), renamed over the deleted torch 2.11 venv on 2026-09-15 — torch 2.11's Inductor cannot compile TF-MossFormer's eval graph at 16 kHz geometry. The 4070 machine deliberately keeps both and has not repeated the swap: `venv/` (torch 2.8.0+cu128, 356 packages) holds the ASR-pipeline extras (whisperx, faster-whisper, ctranslate2, clearvoice, python-pptx, jupyter) that the 2.14 build lacks and still backs the webapp, `asr_pipeline` and the deck build, while `venv_t214/` (torch 2.14.0+cu130, 199 packages) is the one to use for new training there — including anything at 16 kHz geometry. Install those extras into `venv_t214`, constrained to the existing pins, before ever swapping it in. `requirements.txt` is the installable core set (optional extras — mamba-ssm, ASR pipeline, B1 git dependency — are commented blocks); `requirements-freeze.txt` is the exact `pip freeze` of the 3080 venv. Installed on top of the core set there: the webapp/ASR deps (fastapi, uvicorn, python-multipart, pyannote.audio, meeteval, ptflops, num2words), ipykernel/ipywidgets for the notebooks, jiwer, openai-whisper, lxml, black, mypy. Deliberately not carried over from the old venv (nothing in the repo imports them): torchvision, onnx, GitPython, pymupdf/pdfplumber/pypdf. Third-party attribution and vendored-code licences: `THIRD_PARTY.md`. Build gotchas for torch ≥ 2.14: the mamba-ssm / causal-conv1d sdists hard-code `-std=c++17` and ATen now requires C++20 — patch `setup.py` to `-std=c++20` and install with `--no-build-isolation` (official wheels stop at torch 2.10; `setup.sh`'s `MAMBA_FORCE_BUILD` fallback hits the same wall). Both `setup.py` files also hard-code a sm_75…sm_121 `-gencode` list that makes `torch.utils.cpp_extension` ignore `TORCH_CUDA_ARCH_LIST`; on the 3080 (nvcc 13.0.88) that all-arch build segfaulted in `cicc` — delete the list so the env var (`8.6`) is honoured, which also cuts each build to ~2 min.

**SPMamba3** — side project, not in the thesis; its `venv_mamba3/` no longer exists (removed 2026-09-10). Rebuild recipe if ever needed: copy of the main venv + mamba-ssm built from source (`state-spaces/mamba`@0048fbf2) + triton ≥ 3.5 (the SISO kernel needs it); the model code lives on branch `feature/spmamba3-rebuild`.
