...
| `models/tf_mossformer/` | [merlresearch/tf-locoformer](https://github.com/merlresearch/tf-locoformer) (`standalone/tflocoformer_separator.py`), through the in-repo copy fetched 2026-07-18 | Apache-2.0 (MERL) — `models/tf_mossformer/LICENSE` | `tf_locoformer_blocks.py` is the upstream file with four enumerated edits (header): pluggable attention module; `torch.backends.cuda.sdp_kernel` → `torch.nn.attention.sdpa_kernel`; `torch.cuda.amp.autocast` → `torch.amp.autocast("cuda")` on `RMSGroupNorm`; the fp32-forced encoder/decoder cast their activations to the weight dtype so the model runs under bf16 autocast; `local_global_attention.py` and `__init__.py` are original code implementing TF-MossFormer (arXiv:2607.21128), for which no upstream code exists. A second copy on purpose — `models/` must not import from the liftable `asr_pipeline/`. |
...
| pyannote speaker-diarization-3.1 / segmentation-3.0 / wespeaker | `asr_pipeline` diarization (`pyannote` backend) | MIT (gated download) |
...
- `scripts/thesis_figures/data/pipeline_signals_026eafb1__seg00_0-22s.npz` — processed
  signal arrays from a CLARIN-PL fragment. Redistribution terms of the underlying
