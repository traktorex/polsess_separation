...
def test_per_fragment_metrics_counts_match_individual_wrappers():
    pytest.importorskip("meeteval")
    pytest.importorskip("rapidfuzz")
    ref = {"A": [U(0.0, 1.0, "ala ma kota")], "B": [U(1.0, 2.0, "kot ma alę")]}
    hyp = {"A": [U(0.0, 1.0, "ala ma psa")], "B": [U(1.0, 2.0, "kot ma alę")]}
    ...
    # Each sub-result equals the standalone wrapper call, count-for-count.
    assert m["cp"]["cp_errors"] == cpwer_meeteval(ref, hyp, session_id="t")["cp_errors"]
...
def test_per_fragment_metrics_without_mix_omits_mixture_floors():
    pytest.importorskip("meeteval")
    pytest.importorskip("rapidfuzz")
    ref = {"A": [U(0.0, 1.0, "kot")]}
    hyp = {"A": [U(0.0, 1.0, "kot")]}
