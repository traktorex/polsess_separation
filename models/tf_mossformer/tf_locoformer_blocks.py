...
# Vendored from TF-Locoformer (merlresearch/tf-locoformer):
# standalone/tflocoformer_separator.py — copied from the in-repo byte-identical
# vendored copy at asr_pipeline/vendor/tf_locoformer/tflocoformer_separator.py,
# fetched 2026-07-18. This is a SECOND copy on purpose: models/ must never import
# from asr_pipeline/, which is designed to be lifted out of this repository, and
# the asr_pipeline/vendor/ copy stays byte-identical to upstream (its checkpoints
# are sha256-pinned). Local edits live here only.
#
# Local edits (nothing removed):
#   1. The block's self-attention module is pluggable. LocoformerBlock takes an
#      optional `attn_builder(pe) -> nn.Module`; TFLocoformerBlock and
#      TFLocoformerSeparator thread one builder per path (freq / time). Passing
#      none reproduces upstream exactly. models/tf_mossformer/__init__.py passes
#      LocalGlobalMHSA builders, which is the whole architectural diff between
#      TF-Locoformer and TF-MossFormer (arXiv:2607.21128, Fig. 2(b)/(c)).
#   2. `torch.backends.cuda.sdp_kernel(enable_*=...)` (deprecated since torch 2.3)
#      -> `torch.nn.attention.sdpa_kernel([SDPBackend, ...])`, same backend choice
#      expressed as the list the new API takes.
#   3. `@torch.cuda.amp.autocast(enabled=False)` -> `@torch.amp.autocast("cuda",
#      enabled=False)` on RMSGroupNorm.forward; the old spelling raises a
#      FutureWarning at import time under torch 2.8. Semantics unchanged.
#   4. The two fp32-forced encoder/decoder calls now cast their input — and the
#      decoder its output — to the layer's weight dtype. Upstream disables
#      autocast around them but leaves the activation in the autocast dtype, so
#      `self.deconv(batch)` raises "Input type (BFloat16) and bias type (float)
#      should be the same" under this repository's bf16 AMP policy for this model
#      family, and the trailing `torch.complex` rejects bf16 outright. Both casts
#      are no-ops in fp32 and preserve the upstream intent (encoder/decoder and
#      the complex round-trip run in fp32; models/spmamba.py does the same by hand).
#   5. `RotaryEmbedding(..., cache_if_possible=False)`: the library's seq-len cache
#      does an in-place scalar `.item()` check per call, which is a graph break
#      under torch.compile (the same edit models/mossformer2/ carries). Zero
#      parameters, identical outputs; only the compile graph changes.
#
# Licence: Apache-2.0 — see LICENSE in this directory, copied from the upstream
# repository. Original paper: Saijo et al., "TF-Locoformer", IWAENC 2024
# (arXiv:2408.03440).
...
from torch.nn.attention import SDPBackend, sdpa_kernel  # local edit 2
...
class TFLocoformerSeparator(nn.Module):
    ...
    def __init__(
        ...
        # others
        eps: float = 1.0e-5,
        # local edit 1: pluggable self-attention, one builder per TF path.
        # None -> upstream MultiHeadSelfAttention. Each builder is called once
        # per layer as builder(pe) and must return a fresh nn.Module.
        attn_builder_freq=None,
        attn_builder_time=None,
    ):
        super().__init__()
        ...
        if pos_enc == "nope":
            ...
            # cache_if_possible=False: local edit 5 (see header) — the rotary
            # seq-len cache does a `.item()` scalar check that breaks the
            # torch.compile graph inside every attention call.
            pe_freq = RotaryEmbedding(attention_dim // n_heads, cache_if_possible=False)
            pe_time = RotaryEmbedding(attention_dim // n_heads, cache_if_possible=False)
        else:
        ...
        for _ in range(n_layers):
            self.blocks.append(
                TFLocoformerBlock(
                    ...
                    dropout=dropout,
                    eps=eps,
                    attn_builder_freq=attn_builder_freq,
                    attn_builder_time=attn_builder_time,
                )
            )
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        with torch.amp.autocast("cuda", enabled=False):
            # local edit 4: .to(weight dtype) — autocast is off here, so the
            # activation must already match the fp32 weights.
            batch = self.conv(batch.to(self.conv[0].weight.dtype))  # [B, -1, T, F]
        ...
        with torch.amp.autocast("cuda", enabled=False):
            # local edit 4 (see above): the blocks run under autocast, so `batch`
            # arrives in the autocast dtype while self.deconv holds fp32 weights.
            # The output is cast back for the same reason torch.complex is fp32-only
            # below (cf. models/spmamba.py, which handles the identical constraint).
            batch = self.deconv(batch.to(self.deconv.weight.dtype))
            batch = batch.to(self.deconv.weight.dtype)  # [B, num_spk*2, T, F]
...
class TFLocoformerBlock(nn.Module):
    def __init__(
        ...
        dropout=0.0,
        eps=1.0e-5,
        attn_builder_freq=None,  # local edit 1
        attn_builder_time=None,  # local edit 1
    ):
        super().__init__()
        ...
        self.freq_path = LocoformerBlock(
            ...
            dropout=dropout,
            eps=eps,
            attn_builder=attn_builder_freq,
        )
        self.frame_path = LocoformerBlock(
            ...
            dropout=dropout,
            eps=eps,
            attn_builder=attn_builder_time,
        )
...
class LocoformerBlock(nn.Module):
    def __init__(
        ...
        dropout=0.0,
        eps=1.0e-5,
        attn_builder=None,  # local edit 1
    ):
        super().__init__()
        ...
        if attn_builder is None:
            ...
            # local edit 1: caller-supplied attention (TF-MossFormer's
            # LocalGlobalMHSA). It owns its own dims/dropout/window; only the
            # shared positional encoding is handed to it.
            self.attn = attn_builder(pe)
...
class MultiHeadSelfAttention(nn.Module):
    def __init__(
        ...
        # local edit 2: same backend selection as upstream's deprecated
        # torch.backends.cuda.sdp_kernel(enable_*) flags, expressed as the
        # backend list torch.nn.attention.sdpa_kernel takes.
        if flash_attention:
            self.sdpa_backends = [SDPBackend.FLASH_ATTENTION]
        else:
            self.sdpa_backends = [SDPBackend.EFFICIENT_ATTENTION, SDPBackend.MATH]
...
class RMSGroupNorm(nn.Module):
    ...
    @torch.amp.autocast("cuda", enabled=False)  # local edit 3
    def forward(self, input):
