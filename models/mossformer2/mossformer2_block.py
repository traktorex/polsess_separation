...
# train/speech_separation/models/mossformer2/mossformer2_block.py — fetched 2026-06-02.
# Local edits: in-package imports made relative; unused `torchinfo` import removed;
# RotaryEmbedding built with cache_if_possible=False so the model is torch.compile-able
# (the rotary seq-len cache does an in-place scalar copy_ that Inductor cannot lower;
# disabling it is numerically identical).
"""
...
class MossformerBlock_GFSMN(nn.Module):
    def __init__(
        ...
        *,
        ...
        # cache_if_possible=False: the rotary seq-len cache does an in-place scalar
        # copy_ that torch.compile/Inductor cannot lower. Disabling it is numerically
        # identical (freqs are just recomputed per call) and lets the model compile.
        rotary_pos_emb = RotaryEmbedding(dim = min(32, query_key_dim), cache_if_possible = False)
